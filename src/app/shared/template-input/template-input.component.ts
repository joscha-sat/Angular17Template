...
@Component({
  ...
  imports: [MatFormField, MatInput, ReactiveFormsModule, MatFormFieldModule, MatInputModule, MatLabel, TranslocoPipe],
...
export class TemplateInputComponent {
  ...
  readonly appearance: InputSignal<'fill' | 'outline'> = input<'fill' | 'outline'>('outline');
  readonly type: InputSignal<'text' | 'password'> = input<'text' | 'password'>('text');
  readonly subscriptSizing: InputSignal<'dynamic' | 'fixed'> = input<'dynamic' | 'fixed'>('dynamic');
}
